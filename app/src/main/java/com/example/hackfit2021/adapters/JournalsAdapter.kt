...
import kotlinx.android.synthetic.main.journal_prototype.view.*
...
class JournalsAdapter() :
    RecyclerView.Adapter<JournalsAdapter.NotesViewHolder>() {
    var listener: OnItemClickListener? = null
    var arrList = ArrayList<Journals>()
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): NotesViewHolder {
        return NotesViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.journal_prototype,parent,false)
        )
    }
    ...
    fun setData(arrNotesList: List<Journals>){
        arrList = arrNotesList as ArrayList<Journals>
    }
    ...
    override fun onBindViewHolder(holder: NotesViewHolder, position: Int) {
        ...
        holder.itemView.tvDateTime.text = arrList[position].dateTime?.subSequence(0, 10)
        println(arrList[position].dateTime)
